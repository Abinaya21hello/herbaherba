  ...
  const validate = () => {
    ...
    if (updatedUser.address) {
      updatedUser.address.forEach((address, index) => {
        ...
        if (!address.district)
          newErrors[`address.${index}.district`] = "District is required";
        if (!address.state)
          newErrors[`address.${index}.state`] = "State is required";
        if (!address.country)
          newErrors[`address.${index}.country`] = "Country is required";
        if (!address.pincode)
          newErrors[`address.${index}.pincode`] = "Pin Code is required";
  ...
  return (
    <StyledContainer>
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} md={9}>
          <Box display="flex" flexDirection="column" alignItems="center">
            <StyledAvatar
              className="avatar"
              style={{ fontSize: "60px", color: "yellow" }}
            >
              {firstName.charAt(0)}
            </StyledAvatar>
            <Typography
      ...
      >
        ...
        <DialogActions>
          <CancelButton onClick={() => setEditDialogOpen(false)}>
            Cancel
          </CancelButton>
