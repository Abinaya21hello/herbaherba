...
const cartModel = require("../../Models/productModel/cartItemsSchema");
const wishlistModel = require("../../Models/adminModel/wishlistSchema");
...
const deleteProduct = async (req, res) => {
  try {
    ...
    const deletedProduct = await Product.findByIdAndDelete(categoryId);
    ...
    res.status(200).json({
      message: "Product deleted and carts & wishlist updated",
      deletedProduct,
    });
  } catch (err) {
    console.error(err);
...
const deleteModelProduct = async (req, res) => {
  try {
    ...
    await cartModel.updateMany(
      { "cartItems.modelId": modelId },
      { $pull: { cartItems: { modelId: modelId } } }
    );

    await wishlistModel.updateMany(
      { "Wishlist.modelId": modelId },
      { $pull: { Wishlist: { modelId: modelId } } }
    );
...
//  to remove a subProduct from a product model
const deleteModelSubProduct = async (req, res) => {
  try {
    ...
    foundModel.subProduct.pull(subProductId);
    await cartModel.updateMany(
      { "cartItems.subProductId": subProductId },
      { $pull: { cartItems: { subProductId: subProductId } } }
    );

    await wishlistModel.updateMany(
      { "Wishlist.subProductId": subProductId },
      { $pull: { Wishlist: { subProductId: subProductId } } }
    );
    await product.save();

    res.status(200).json({ message: "Sub-product deleted" });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
  }
