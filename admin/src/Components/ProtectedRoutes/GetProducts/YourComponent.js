...
const YourComponent = () => {
  ...
  const handleDeleteClick = async (itemId, type) => {
    try {
      let apiUrl = "";
      if (type === "Category") {
        apiUrl = `api/category-delete/${itemId}`;
      } else {
        apiUrl = `api/sub-product-delete/${selectedCategory}/models/${selectedMainProduct}/sub/${itemId}`;
      }
  ...
  const handleEditSave = async () => {
    try {
      ...
      if (editData.type === "Category") {
        ...
        apiUrl = `api/sub-product-update/${selectedCategory}/models/${selectedMainProduct}/sub/${editData._id}`;
      }
  ...
  const handleUpdateCategory = async () => {
    try {
     ...
     `api/category-update/${dialogCategory._id}`,
  ...
  const handleCategoryModelDelete = async (modelId) => {
    try {
      const apiUrl = `api/products-delete/${selectedCategory}/models/${modelId}`;
  ...
  const handleCategoryModelUpdate = async () => {
    try {
      ...
      const apiUrl = `api/products-update/${selectedCategory}/models/${dialogModel._id}`;

      await axiosInstance.put(apiUrl, updatedModel, {
        withCredentials: true,
      });
