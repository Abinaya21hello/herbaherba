...
const AddProduct = () => {
  ...
  const handleSubProductChange = (field, value) => {
    ...
    if (field === "briefDescription") {
      if (stringValue.length < 350 || stringValue.length > 1000) {
        setBriefDescError(
          "Brief Description must be between 350 and 1000 characters."
        );
      } else {
        setBriefDescError("");
      }
  ...
  const handleCategoryChange = (e) => {
    ...
    if (value.length < 3 || value.length > 20) {
      setCategoryError("Category must be between 3 and 20 characters.");
  ...
  const handleMainProductChange = (e) => {
    ...
    if (value.length < 3 || value.length > 30) {
      setMainProductError("Main Product must be between 3 and 30 characters.");
    } else if (/[^A-Za-z\s]/.test(value)) {
  ...
  const handleSubProductInputChange = (field, value) => {
    if (value.length < 3 || value.length > 40) {
      setBriefDescError(
        "Sub Product Name and Short Description must be between 3 and 30 characters."
      );
    } else if (/[^A-Za-z\s]/.test(value)) {
      setBriefDescError(
        "Sub Product Name and Short Description cannot contain special characters."
      );
    } else {
      setBriefDescError("");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      currentSubProduct.briefDescription.length < 100 ||
      currentSubProduct.briefDescription.length > 1000
    ) {
      setBriefDescError(
        "Brief Description must be between 100 and 500 characters."
      );
      return;
    }

    if (
      parseFloat(currentSubProduct.offerPrice) >=
      parseFloat(currentSubProduct.currentPrice)
    ) {
    ...
    formData.append(
      "models",
      JSON.stringify([{ mainProduct, subProduct: [currentSubProduct] }])
    );
  ...
  return (
    <Container maxWidth="md">
      ...
      {error && <Alert severity="error">Upload Image Below 3 mb</Alert>}
          ...
          <Grid container spacing={2}>
              ...
              <TextField
                ...
                InputProps={{
                  inputProps: {
                    pattern: "[A-Za-z ]{3,20}",
                    title:
                      "Only alphabetic characters are allowed and length must be between 3 and 20 characters",
                  },
                }}
              />
            </Grid>
            ...
            <Grid item xs={12} sm={4}>
              <TextField
                ...
                onChange={(e) =>
                  handleSubProductInputChange("subproductname", e.target.value)
                }
                required
                InputProps={{
                  inputProps: {
                    pattern: "[A-Za-z ]{3,25}",
                    title:
                      "Only alphabetic characters are allowed and length must be between 3 and 25 characters",
                  },
                }}
              />
            </Grid>
              ...
              <TextField
                ...
                InputProps={{
                  inputProps: {
                    pattern: "[A-Za-z ]{3,50}",
                    title:
                      "Only alphabetic characters are allowed and length must be between 3 and 15 characters",
                  },
                }}
              />
            </Grid> */}
            <Grid item xs={12} sm={12}>
              <TextField
                ...
                onChange={(e) =>
                  handleSubProductChange("briefDescription", e.target.value)
                }
            ...
            <Grid item xs={12} sm={4}>
              <TextField
                ...
                onChange={(e) =>
                  handleSubProductChange("ratings", e.target.value)
                }
                type="number"
                InputProps={{
            ...
            <Grid item xs={12} sm={4}>
              ...
              <CustomFileInputLabel
                ...
                style={{ color: "#000000", backgroundColor: "#F9FBE7" }}
              >
              ...
              {imageSrc && (
                <img
                  ...
                  style={{
                    maxWidth: "100%",
                    filter: "brightness(1.7)", // Adjust brightness
                    imageRendering: "crisp-edges", // Enhance sharpness for HD look
                  }}
                />
              )}
            </Grid>
